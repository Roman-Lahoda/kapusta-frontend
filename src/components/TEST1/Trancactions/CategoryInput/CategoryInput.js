  ...
  return (
    <CategoryContainer>
      <Input
        ...
        readOnly
        value={categoryPick}
        placeholder={type === 'Expenses' ? 'Категория товара' : 'Категория дохода'}
        onClick={handleClick}
        onFocus={handleClick}
      />
      {!isCategories || (
      ...
      {!isCategories ? (
        <ArrowDown onClick={handleClick} onFocus={handleClick} id="arrow">
          <IoIosArrowDown color="#C7CCDC" width="12px" height="8" />
        </ArrowDown>
      ) : (
        <ArrowUp onClick={handleClick} onFocus={handleClick} id="arrow">
          <IoIosArrowUp color="#C7CCDC" width="12px" height="8" />
        </ArrowUp>
      )}
    </CategoryContainer>
  );
