...
function UniversalModal({ text, onClickYes, onClose, active }) {
  return (
      <div className={active ? `${s.modal} ${s.open}` : `${s.modalNone}`}>
        ...
        <ul className={s.list}>
          <li className={s.item}>
            <button type='button' className={`${s.button} ${s.buttonYes}`} onClick={onClickYes}>Да</button>
          </li>
          <li>
            <button type='button' className={`${s.button} ${s.buttonNo}`} onClick={onClose}>Нет</button>
          </li>
        </ul>
